...
// node_modules/@shocknet/clink-sdk/build/sender.js
var { getConversationKey: getConversationKey2, decrypt: decrypt3 } = nip44_exports;
var sendRequest = async (pool, pair, relays, toPub, e, kindExpected, timeoutSeconds, moreCb) => {
  ...
  return new Promise((res, rej) => {
    ...
    try {
      closer = pool.subscribeMany(relays, [filter], {
        onevent: async (e2) => {
          console.log(`[ClinkSDK] Received response event: kind=${e2.kind}, eventId=${e2.id}, from=${e2.pubkey}`);
          if (timer)
            clearTimeout(timer);
          const content = decrypt3(e2.content, getConversationKey2(pair.privateKey, toPub));
          if (!resolved) {
            resolved = true;
...
var newFilter = (publicKey, eventId, kindExpected) => ({
...
// node_modules/@shocknet/clink-sdk/build/noffer.js
var { getConversationKey: getConversationKey3, encrypt: encrypt3 } = nip44_exports;
var SendNofferRequest = async (pool, privateKey, relays, toPubKey, data, timeoutSeconds = 30, onReceipt) => {
  if (data.description && data.description.length > 100) {
    throw new Error("Description must be less than 100 characters");
  }
  const publicKey = getPublicKey(privateKey);
  const content = encrypt3(JSON.stringify(data), getConversationKey3(privateKey, toPubKey));
...
var newNofferEvent = (content, fromPub, toPub) => ({
  content,
  created_at: Math.floor(Date.now() / 1e3),
  kind: 21001,
  pubkey: fromPub,
...
// node_modules/@shocknet/clink-sdk/build/ndebit.js
var { getConversationKey: getConversationKey4, decrypt: decrypt4, encrypt: encrypt4 } = nip44_exports;
var SendNdebitRequest = async (pool, privateKey, relays, toPubKey, data, timeoutSeconds) => {
  const publicKey = getPublicKey(privateKey);
  const content = encrypt4(JSON.stringify(data), getConversationKey4(privateKey, toPubKey));
...
var newNdebitPaymentRequest = (invoice, amount, pointer) => {
  return {
    bolt11: invoice,
    amount_sats: amount,
    pointer
...
var newNdebitBudgetRequest = (frequency, amount, pointer) => {
  return {
    amount_sats: amount,
    frequency,
    pointer
...
var newNdebitEvent = (content, fromPub, toPub) => ({
  content,
  created_at: Math.floor(Date.now() / 1e3),
  kind: 21002,
  pubkey: fromPub,
...
// node_modules/@shocknet/clink-sdk/build/nmanage.js
var { getConversationKey: getConversationKey5, decrypt: decrypt5, encrypt: encrypt5 } = nip44_exports;
var SendNmanageRequest = async (pool, privateKey, relays, toPubKey, data, timeoutSeconds) => {
  const publicKey = getPublicKey(privateKey);
  const content = encrypt5(JSON.stringify(data), getConversationKey5(privateKey, toPubKey));
...
var newNmanageEvent = (content, fromPub, toPub) => ({
  content,
  created_at: Math.floor(Date.now() / 1e3),
  kind: 21003,
  pubkey: fromPub,
...
// node_modules/@shocknet/clink-sdk/build/nip19Extension.js
var utf8Decoder2 = new TextDecoder("utf-8");
var utf8Encoder2 = new TextEncoder();
var Bech32MaxSize2 = 5e3;
var OfferPriceType;
(function(OfferPriceType2) {
    ...
    case "noffer": {
      ...
      return {
        ...
        data: {
          ...
          relay: utf8Decoder2.decode(tlv[1][0]),
          offer: utf8Decoder2.decode(tlv[2][0]),
          priceType: tlv[3][0][0],
          price: tlv[4] ? parseInt(bytesToHex3(tlv[4][0]), 16) : void 0
        }
      };
    }
    case "ndebit": {
      ...
      return {
        ...
        data: {
          pubkey: bytesToHex3(tlv[0][0]),
          relay: utf8Decoder2.decode(tlv[1][0]),
          pointer: tlv[2] ? utf8Decoder2.decode(tlv[2][0]) : void 0
        }
      };
    }
    case "nmanage": {
...
// node_modules/@shocknet/clink-sdk/build/index.js
var ClinkSDK = class {
...
ClinkSDK.decodeBech32 = decodeBech32;
ClinkSDK.generateSecretKey = generateSecretKey;
ClinkSDK.newListRequest = newListRequest;
ClinkSDK.newNdebitBudgetRequest = newNdebitBudgetRequest;

// nostr/clink-bridge.mjs
var { getConversationKey: getConversationKey6, encrypt: encrypt6, decrypt: decrypt6 } = nip44_exports;
async function requestInvoice(noffer, amountSats, description, expiresInSeconds, timeoutSeconds, additionalRelays) {
  ...
  const content = encrypt6(JSON.stringify(data), getConversationKey6(pk, toPub));
  const event = {
    content,
    created_at: Math.floor(Date.now() / 1e3),
    kind: 21001,
  ...
  return new Promise((resolve, reject) => {
    ...
    const closer = pool.subscribeMany(relays, [filter], {
      onevent: (e) => {
        if (!resolved) {
          try {
            const decrypted = decrypt6(e.content, getConversationKey6(pk, toPub));
  ...
  return new Promise((resolve) => {
    ...
    const closer = pool.subscribeMany(relays, [filter], {
      onevent: (e) => {
        if (!resolved) {
          try {
            const decrypted = decrypt6(e.content, getConversationKey6(privateKey, toPub));
